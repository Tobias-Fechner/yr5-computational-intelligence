...
def population(count, min, max, length):
    """
    ...
    """
    return [ individual(min, max, length) for x in range(count) ]
...
def fitness(individual):
    ...
    for x in np.linspace(-1,1, 20):
        yTarget = 25*x**5 + 18*x**4 + 0 - 14*x**2 + 7*x -19
        yPredict = a1*x**5 + a2*x**4 + a3*x**3 + a4*x**2 + a5*x + c

        # Store target and prediction as pair in list of tuples
        y.append((yTarget, yPredict))

    # Iterate over list of tuples to calculate square of error between target and prediction
    for target, prediction in y:
        error = abs(target - prediction)
        #squared = error**2

        # Store square of error in list
        errors.append(error)

    # Return mean of squared errors as fitness value for individual
...
def grade(population):
    ...
    :return:
    """
    return np.average([fitness(x) for x in population])

def evolve(population, retain=0.2, random_select=0.05, mutate=0.01, femalePortion=0.5):
    ...
    individualGrades = [(fitness(x), list(x)) for x in population]
